...
- **Close stops the Host.** Closing the window quits the shell and first stops the Web Host through `scripts/stop-web.ps1`, so an app-style exit leaves no orphaned server. The stop runs synchronously inside `before-quit`: a detached child would die with Electron's Windows Job Object when the process exits, so the script must finish while the shell is still alive. `--smoke` uses `app.exit` and never stops the Host.
